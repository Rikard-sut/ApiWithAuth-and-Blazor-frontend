...
namespace Infrastructure.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<JwtSecurityToken> Login(LoginUserCommand command)
        {
            var authSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(command.Config["JWT:Secret"]));
            ...
            if (user != null && await _userManager.CheckPasswordAsync(user, command.Password))
            {
                ...
                var token = new JwtSecurityToken(
                    issuer: command.Config["JWT:ValidIssuer"],
                    audience: command.Config["JWT:ValidAudience"],
        ...
        public async Task<RegisterUserResponse> RegisterAdmin(RegisterAdminCommand model)
        {
            //Finds the user if it exists in the database and returns proper errormessage.
